...
def preprocess_image(path, id, size=256, gaussian_blur_kernel_size=(1, 1)):
    ...
    # create dot mask
    target = create_dot_mask(orig_img, circles, size)

    # apply gaussian blur
    blurred_img = cv2.GaussianBlur(target, gaussian_blur_kernel_size, 1, 1)

    return resized_image, blurred_img
